...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // slider function
    function initializeSlider(container, regionDataKey) {
    ...
    actionAfterContainer.addEventListener('click', function(event) {
      ...
      const cleanRegionName = regionName.replace(/,$/, ''); 

      const regionDataKey = regionId
          .split('_') 
          .map(word => word.replace(/(?:^|-)(.)/g, (match, char) => match.toUpperCase()))
          .join(' '); 
      console.log(regionDataKey)
      ...
      const detailedInfoHTML = `
            <div class="region-detailed-info">
                ...
                <div class="region-forecast">
                    <div class="region-forecast__info">
                        ...
                        <div class="forecast-id">
                            <div class="region-entity__info">
                                <span id="${regionId}-display" class="region-entity__name">${cleanRegionName}</span>
                                <span class="region-entity__type"></span> </div>
                            <span class="region-entity__support">Apr. 2</span> </div>
                    </div>
                    <div class="region-forecast__holder">
                    <div class="region-forecast__slider forecast-view-active"> <div class="time-slider-container" id="${regionId}--details">
                    ...
                    <div class="region-forecast__table"> <div class="hour-blobholder">
                            ${Array.from({ length: 12 }, (_, i) => `<div class="hour-blob">${String(i).padStart(2, '0')}</div>`).join('')}
                        </div>
                        <div class="hour-blobholder">
                            ${Array.from({ length: 12 }, (_, i) => `<div class="hour-blob">${String(i + 12).padStart(2, '0')}</div>`).join('')}
                        </div>
                    </div>
                    </div>
                    <div class="region-forecast__stats">
                        ...
                        <div class="region-return-to-list">WANNA SEE HOURLY VIEW?</div> </div>
                </div>
            </div>`;
      ...
      const detailedView = mainContainer.querySelector('.region-detailed-info:last-child');
      if (detailedView && timeData && timeData[regionDataKey]) {
          const forecastDataForRegion = timeData[regionDataKey];
          const hourBlobs = detailedView.querySelectorAll('.region-forecast__table .hour-blob');

          hourBlobs.forEach(blob => {
              const hour = blob.textContent; 
              const timeKey = `${hour}:00`; 
              if (forecastDataForRegion[timeKey] === true) {
                  blob.classList.add('hour-blob--active');
              } else {
                    blob.classList.remove('hour-blob--active'); 
              }
          });
      } else if (!timeData || !timeData[regionDataKey]) {
            console.warn(`Forecast data missing for region key: ${regionDataKey}`);
      }
      ...
      if (newSliderContainer) { 
          initializeSlider(newSliderContainer, regionDataKey); 
      }


      // update date display 
      const supportElement = detailedView?.querySelector(".region-entity__support"); 
      if (supportElement) {
          const today = new Date();
          const monthNames = ["Jan.", "Feb.", "Mar.", "Apr.", "May", "Jun.", "Jul.", "Aug.", "Sep.", "Oct.", "Nov.", "Dec."];
          const formattedDate = `${monthNames[today.getMonth()]} ${today.getDate()}`;
          supportElement.textContent = formattedDate; // Just set the date
      }

  });


  // click listener for view toggle and global reset
  mainContainer.addEventListener('click', function (event) {
      const returnToListButton = event.target.closest('.region-return-to-list');
      const separatorElement = event.target.closest('.separator');

      if (returnToListButton) {
          const detailedView = returnToListButton.closest('.region-detailed-info');
          if (!detailedView) return;

          const sliderView = detailedView.querySelector('.region-forecast__slider');
          const tableView = detailedView.querySelector('.region-forecast__table');
          const button = returnToListButton; // The button itself

          if (sliderView && tableView) {
              const isSliderActive = sliderView.classList.contains('forecast-view-active');

              sliderView.classList.toggle('forecast-view-active');
              tableView.classList.toggle('forecast-view-active');

              if (tableView.classList.contains('forecast-view-active')) {
                  button.textContent = 'WANNA SEE SLIDER VIEW?';
              } else {
                  button.textContent = 'WANNA SEE HOURLY VIEW?';
              }
            }
